...
- **범위**: `exe_type = INDIVIDUAL` 또는 `BATCH`인 경우만 처리 (WCS는 OMS에서 직접 API 호출)
...
웨이브 인계 → SKU별 합산 피킹 지시 생성 (N 주문 = 1 피킹 지시)
  → 작업자 할당 → 합산 피킹(카트/컨베이어)
  → 포장 스테이션에서 주문별 분배(DPS/수작업)
  → 검수/포장 → 라벨 → 출하
```
...
| 3 | `packing_orders` | PackingOrder | 검수/포장 지시 (헤더) | 실테이블 |
| 4 | `packing_order_items` | PackingOrderItem | 검수/포장 상세 | 실테이블 |
| 5 | `packing_boxes` | PackingBox | 포장 박스 | 실테이블 |
| 6 | `shipping_orders` | ShippingOrder | 출하 지시 | 실테이블 |
| 7 | `shipping_order_items` | ShippingOrderItem | 출하 상세 | 실테이블 |
| 8 | `fulfillment_progress` | FulfillmentProgress | 풀필먼트 진행 현황 뷰 | 뷰 (ignoreDdl) |
...
[피킹 지시 생성]                                  ← createPickingTasks()
    ...
    ↓
[작업자 할당 (→ ASSIGNED)]                        ← assignPickingTask()
    ├─ 작업자 코드 매핑
    └─ PDA/RF에 작업 표시
    ↓
[피킹 실행 (→ IN_PROGRESS)]                      ← startPickingTask()
    ├─ PDA 바코드 스캔으로 피킹 확인
...
[포장 완료 (PackingOrder → COMPLETED)]            ← completePackingOrder()
    ...
    └─ 자동 출하 지시 생성 (설정 시)
    ↓
[출하 지시 (ShippingOrder)]                       ← createShippingOrder()
    ├─ 택배사/송장번호 매핑
    ├─ 운송장 라벨 출력
    └─ 상태: CREATED → LABEL_PRINTED
    ↓
...
[합산 피킹 지시 생성]                              ← createBatchPickingTask()
    ...
    └─ PickingStartedEvent 발행 (waveNo 단위)
    ↓
[합산 피킹 실행]                                   ← startPickingTask()
...
```
[OMS 인계 (RELEASED, exe_type=INDIVIDUAL)]
    ↓
[피킹 지시 생성 (개별)]                            ← createPickingTasks()
    ├─ 대량 단일 주문 → 1건의 PickingTask
    ├─ 팔레트 단위 피킹
    └─ PickingStartedEvent 발행
    ↓
[피킹 실행 (PDA)]                                  ← startPickingTask()
    ├─ 로케이션 → 팔레트/박스 단위 피킹
    └─ 실수량 기록
    ↓
...
| 상태 | 코드 | 설명 | 주체 |
|------|------|------|------|
| 생성 | `CREATED` | 피킹 지시 생성, 작업자 미배정 | 시스템 |
| 배정 | `ASSIGNED` | 작업자 배정 완료, 작업 대기 | 관리자/시스템 |
...
### 4.5 출하 지시 (ShippingOrder) 상태

```
CREATED ─→ LABEL_PRINTED ─→ MANIFESTED ─→ SHIPPED
   │              │                │
   │              │                └── confirmShipping()
   │              └──────────────── createManifest()
   └──────────────────────────────── printLabel()

 * CANCELLED: cancelShippingOrder()
```

| 상태 | 코드 | 설명 | 주체 |
|------|------|------|------|
| 생성 | `CREATED` | 출하 지시 생성 | 시스템 |
| 라벨 출력 | `LABEL_PRINTED` | 운송장/라벨 출력 완료 | 시스템 |
| 적하 목록 | `MANIFESTED` | 적하 목록(매니페스트) 전송 완료 | 시스템 |
| 출하 완료 | `SHIPPED` | 최종 출하 확정 (차량 상차 완료) | 작업자/시스템 |
| 취소 | `CANCELLED` | 출하 취소 | 관리자 |
...
| `pick_method` | VARCHAR | Y | 20 | 피킹 방식 (CART/RF/DPS/PAPER) |
| `exe_type` | VARCHAR | Y | 20 | 실행 유형 (INDIVIDUAL/BATCH) |
| `zone_cd` | VARCHAR | Y | 30 | 존 코드 (존 피킹 시) |
| `assigned_worker_cd` | VARCHAR | Y | 30 | 배정 작업자 코드 |
| `assigned_worker_nm` | VARCHAR | Y | 100 | 배정 작업자 명 |
...
| `short_total_qty` | DOUBLE | Y | - | 부족 총 수량 |
| `progress_rate` | DOUBLE | Y | - | 진행률 (%) |
| `status` | VARCHAR | N | 20 | 상태 (CREATED/ASSIGNED/IN_PROGRESS/COMPLETED/CANCELLED) |
...
| `ix_picking_tasks_5` | `domain_id, assigned_worker_cd, status` | N |
...
| `box_in_qty` | DOUBLE | Y | - | 박스 입수 수량 |
| `order_qty` | DOUBLE | N | - | 지시 수량 |
| `order_box` | INTEGER | Y | - | 지시 박스 수 |
| `order_ea` | DOUBLE | Y | - | 지시 낱개 수 |
| `pick_qty` | DOUBLE | Y | - | 실적 수량 (기본값 0) |
| `pick_box` | INTEGER | Y | - | 실적 박스 수 (기본값 0) |
| `pick_ea` | DOUBLE | Y | - | 실적 낱개 수 (기본값 0) |
| `short_qty` | DOUBLE | Y | - | 부족 수량 (기본값 0) |
| `status` | VARCHAR | Y | 10 | 상태 (WAIT/RUN/PICKED/SHORT/CANCEL) |
...
| `worker_cd` | VARCHAR | Y | 30 | 작업자 코드 |
| `plan_sku_count` | INTEGER | Y | - | 계획 SKU 수 |
| `plan_total_qty` | DOUBLE | Y | - | 계획 총 수량 |
| `result_sku_count` | INTEGER | Y | - | 실적 SKU 수 |
| `result_total_qty` | DOUBLE | Y | - | 실적 총 수량 |
| `total_box_count` | INTEGER | Y | - | 포장 박스 수 |
...
| `id` | VARCHAR | N | 40 | PK (UUID) |
| `packing_order_id` | VARCHAR | N | 40 | FK → packing_orders |
| `box_no` | INTEGER | N | - | 박스 순번 (포장 지시 내 UNIQUE) |
| `box_type` | VARCHAR | Y | 20 | 박스 유형 (BOX/BAG/ENVELOPE/PALLET) |
| `box_wt` | DOUBLE | Y | - | 박스 중량 (kg) |
| `box_length` | DOUBLE | Y | - | 박스 길이 (cm) |
| `box_width` | DOUBLE | Y | - | 박스 너비 (cm) |
| `box_height` | DOUBLE | Y | - | 박스 높이 (cm) |
| `total_item_count` | INTEGER | Y | - | 포함 아이템 종 수 |
...
| 인덱스명 | 컬럼 | 유니크 |
|----------|------|--------|
| `ix_packing_boxes_0` | `domain_id, packing_order_id, box_no` | Y |
| `ix_packing_boxes_1` | `domain_id, packing_order_id` | N |
| `ix_packing_boxes_2` | `domain_id, status` | N |

---

### 5.6 shipping_orders — 출하 지시

포장 완료 후 생성되는 최종 출하 확정 단위. ShipmentOrder와 1:1 관계.

**Lifecycle**:
- `@PrePersist` — `ship_order_no` 자동 채번, `status` 기본값 `CREATED`

| 컬럼명 | 타입 | Null | 길이 | 설명 |
|--------|------|------|------|------|
| `id` | VARCHAR | N | 40 | PK (UUID) |
| `ship_order_no` | VARCHAR | N | 30 | 출하 지시 번호 (자동 채번, UNIQUE) |
| `shipment_order_id` | VARCHAR | N | 40 | FK → shipment_orders (OMS) |
| `shipment_no` | VARCHAR | N | 30 | 출하 주문 번호 (OMS) |
| `pack_order_no` | VARCHAR | Y | 30 | 포장 지시 번호 |
| `wave_no` | VARCHAR | Y | 30 | 웨이브 번호 |
| `order_date` | VARCHAR | N | 10 | 작업 일자 |
| `com_cd` | VARCHAR | N | 30 | 화주사 코드 |
| `wh_cd` | VARCHAR | N | 30 | 창고 코드 |
| `carrier_cd` | VARCHAR | Y | 30 | 택배사 코드 |
| `invoice_no` | VARCHAR | Y | 50 | 송장 번호 |
| `total_box_count` | INTEGER | Y | - | 박스 수 |
| `total_qty` | DOUBLE | Y | - | 총 출하 수량 |
| `total_wt` | DOUBLE | Y | - | 총 중량 (kg) |
| `dock_cd` | VARCHAR | Y | 30 | 출하 도크 코드 |
| `vehicle_no` | VARCHAR | Y | 30 | 차량 번호 |
| `status` | VARCHAR | N | 20 | 상태 (CREATED/LABEL_PRINTED/MANIFESTED/SHIPPED/CANCELLED) |
...
| 인덱스명 | 컬럼 | 유니크 |
|----------|------|--------|
| `ix_shipping_orders_0` | `domain_id, ship_order_no` | Y |
| `ix_shipping_orders_1` | `domain_id, shipment_order_id` | Y |
| `ix_shipping_orders_2` | `domain_id, shipment_no` | N |
| `ix_shipping_orders_3` | `domain_id, wave_no` | N |
| `ix_shipping_orders_4` | `domain_id, order_date, status` | N |
| `ix_shipping_orders_5` | `domain_id, carrier_cd, status` | N |
| `ix_shipping_orders_6` | `domain_id, invoice_no` | N |
| `ix_shipping_orders_7` | `domain_id, dock_cd, status` | N |

---

### 5.7 shipping_order_items — 출하 상세

출하 지시의 SKU별 상세. 박스별 출하 수량을 관리.

| 컬럼명 | 타입 | Null | 길이 | 설명 |
|--------|------|------|------|------|
| `id` | VARCHAR | N | 40 | PK (UUID) |
| `shipping_order_id` | VARCHAR | N | 40 | FK → shipping_orders |
| `shipment_order_item_id` | VARCHAR | Y | 40 | FK → shipment_order_items (OMS) |
| `packing_box_id` | VARCHAR | Y | 40 | FK → packing_boxes |
| `sku_cd` | VARCHAR | N | 30 | 상품 코드 |
| `sku_nm` | VARCHAR | Y | 100 | 상품명 |
| `barcode` | VARCHAR | Y | 50 | 바코드 |
| `lot_no` | VARCHAR | Y | 50 | 로트 번호 |
| `expired_date` | VARCHAR | Y | 10 | 유통기한 |
| `shipped_qty` | DOUBLE | N | - | 출하 수량 |
...
| 인덱스명 | 컬럼 | 유니크 |
|----------|------|--------|
| `ix_shipping_order_items_0` | `domain_id, shipping_order_id` | N |
| `ix_shipping_order_items_1` | `domain_id, shipping_order_id, sku_cd` | N |
| `ix_shipping_order_items_2` | `domain_id, packing_box_id` | N |
| `ix_shipping_order_items_3` | `domain_id, shipment_order_item_id` | N |

---

### 5.8 fulfillment_progress — 풀필먼트 진행 현황 뷰
...
SELECT
    ...
    pt.order_date,
    pt.com_cd, pt.wh_cd,
    pt.pick_type, pt.pick_method, pt.exe_type,
    pt.assigned_worker_cd,
    pt.plan_order_count, pt.plan_sku_count, pt.plan_total_qty,
    pt.result_total_qty AS pick_result_qty,
    pt.short_total_qty,
    pt.progress_rate AS pick_progress_rate,
    ...
    so2.ship_order_no,
    so2.carrier_cd, so2.invoice_no,
    so2.total_qty AS ship_total_qty,
    so2.dock_cd,
    so2.status AS ship_status,
    so2.shipped_at,
    pt.domain_id,
...
FROM
    ...
    LEFT OUTER JOIN packing_orders po
        ...
        AND pt.shipment_order_id = po.shipment_order_id
    LEFT OUTER JOIN shipping_orders so2
        ON pt.domain_id = so2.domain_id
        AND pt.shipment_order_id = so2.shipment_order_id
```
...
│                     │      │                      │
│ pack_order_no (UNQ) │      │ box_no               │
│ pick_task_no        │      │ box_type             │
...
│                     │
│ ship_order_no (UNQ) │
│ shipment_order_id   │──→ OMS shipment_orders
│ pack_order_no       │──→ packing_orders
│ carrier_cd          │
│ invoice_no          │
│ dock_cd             │
│ status              │
...
│                     │
│ packing_box_id (FK) │──→ packing_boxes
│ sku_cd / barcode    │
│ shipped_qty         │
│ status              │
└─────────────────────┘
...
| 코드 | 값 | 설명 |
|------|-----|------|
| `PICK_METHOD_CART` | `CART` | 카트 피킹 (다주문 동시 피킹) |
| `PICK_METHOD_RF` | `RF` | RF/PDA 기반 단건 피킹 |
| `PICK_METHOD_DPS` | `DPS` | DPS(표시기) 기반 피킹 |
| `PICK_METHOD_PAPER` | `PAPER` | 피킹 리스트 종이 출력 |
...
### 7.9 출하 지시 상태 (ShippingOrder Status)

| 코드 | 값 | 설명 |
|------|-----|------|
| `STATUS_CREATED` | `CREATED` | 생성 |
| `STATUS_LABEL_PRINTED` | `LABEL_PRINTED` | 라벨 출력 완료 |
| `STATUS_MANIFESTED` | `MANIFESTED` | 적하 목록 전송 완료 |
| `STATUS_SHIPPED` | `SHIPPED` | 출하 완료 |
| `STATUS_CANCELLED` | `CANCELLED` | 취소 |
...
| 설정키 | 기본값 | 설명 |
|--------|--------|------|
| `ful.picking.default-method` | `RF` | 기본 피킹 방식 (CART/RF/DPS/PAPER) |
| `ful.picking.auto-assign.enabled` | `false` | 피킹 지시 생성 시 자동 작업자 배정 |
| `ful.picking.auto-start.on-assign` | `false` | 작업자 배정 시 자동 시작 |
...
| Method | URL | 설명 |
|--------|-----|------|
| POST | `picking_tasks/create` | 피킹 지시 생성 (OMS 웨이브 인계) |
| POST | `picking_tasks/{id}/assign` | 작업자 배정 (CREATED → ASSIGNED) |
| POST | `picking_tasks/assign_batch` | 복수 피킹 지시 일괄 배정 |
| POST | `picking_tasks/{id}/start` | 피킹 시작 (ASSIGNED → IN_PROGRESS) |
| POST | `picking_tasks/{id}/items/{itemId}/pick` | 개별 아이템 피킹 확인 (스캔) |
| POST | `picking_tasks/{id}/items/{itemId}/short` | 피킹 부족 처리 |
| POST | `picking_tasks/{id}/complete` | 피킹 완료 (IN_PROGRESS → COMPLETED) |
| POST | `picking_tasks/{id}/cancel` | 피킹 취소 (→ CANCELLED) |
| GET | `picking_tasks/{id}/items` | 피킹 항목 목록 조회 |
| GET | `picking_tasks/worker/{workerCd}` | 작업자별 할당 목록 조회 |

### 9.2 검수/포장 트랜잭션 API

| Method | URL | 설명 |
|--------|-----|------|
| POST | `packing_orders/create` | 포장 지시 생성 (피킹 완료 후) |
| POST | `packing_orders/{id}/start` | 검수/포장 시작 (CREATED → IN_PROGRESS) |
| POST | `packing_orders/{id}/items/{itemId}/inspect` | 아이템 검수 (수량 대조) |
| POST | `packing_orders/{id}/items/{itemId}/pack` | 아이템 포장 (박스 투입) |
| POST | `packing_orders/{id}/boxes/create` | 박스 생성 (OPEN) |
| POST | `packing_orders/{id}/boxes/{boxId}/close` | 박스 닫기 (OPEN → CLOSED) |
| POST | `packing_orders/{id}/complete` | 포장 완료 (IN_PROGRESS → COMPLETED) |
| POST | `packing_orders/{id}/cancel` | 포장 취소 (→ CANCELLED) |
| GET | `packing_orders/{id}/items` | 포장 항목 목록 조회 |
| GET | `packing_orders/{id}/boxes` | 포장 박스 목록 조회 |

### 9.3 출하 트랜잭션 API

| Method | URL | 설명 |
|--------|-----|------|
| POST | `shipping_orders/create` | 출하 지시 생성 (포장 완료 후) |
| POST | `shipping_orders/{id}/print_label` | 운송장 라벨 출력 (CREATED → LABEL_PRINTED) |
| POST | `shipping_orders/{id}/manifest` | 적하 목록 전송 (LABEL_PRINTED → MANIFESTED) |
| POST | `shipping_orders/{id}/confirm` | 출하 확정 (→ SHIPPED) |
| POST | `shipping_orders/confirm_batch` | 복수 출하 일괄 확정 |
| POST | `shipping_orders/{id}/cancel` | 출하 취소 (→ CANCELLED) |
| POST | `shipping_orders/{id}/update_invoice` | 송장 번호 업데이트 |
| GET | `shipping_orders/{id}/items` | 출하 항목 조회 |

### 9.4 대시보드/현황 API

| Method | URL | 설명 |
|--------|-----|------|
| GET | `dashboard/picking_status` | 피킹 진행 현황 (상태별 건수) |
| GET | `dashboard/packing_status` | 검수/포장 진행 현황 |
| GET | `dashboard/shipping_status` | 출하 진행 현황 |
| GET | `dashboard/worker_performance` | 작업자별 실적 (피킹 건수, 수량, 시간) |
| GET | `dashboard/wave_progress/{waveNo}` | 웨이브별 풀필먼트 진행률 |
| GET | `dashboard/dock_status` | 도크별 출하 현황 |
...
| ShippingOrder | `/rest/shipping_orders` | 출하 지시 CRUD |
| ShippingOrderItem | 상위에 포함 | `{id}/items`로 조회 |
...
1. 피킹 지시 생성 시
   ...
   └─ inventories.lastTranCd: PICK_START
...
2. 출하 취소 시
   ...
   ├─ inventories.lastTranCd: OUT_CANCEL
   └─ InventoryHist 기록 생성
```
...
public class FulfillmentReleaseRequest {
    ...
    private String exeType;                    // 실행 유형 (INDIVIDUAL/BATCH)
...
|--------|----------|----------|--------------|
| `PickingStartedEvent` | 피킹 지시 최초 스캔 시 | shipmentOrderId, waveNo, pickerId, startedAt | RELEASED → PICKING |
| `PackingStartedEvent` | 검수/포장 시작 시 | shipmentOrderId, waveNo, packerId, startedAt | PICKING → PACKING |
| `ShipmentCompletedEvent` | 출하 확정 시 | shipmentOrderId, invoiceNo, carrierCd, shippedAt | PACKING → SHIPPED |
...
public class FulfillmentEventPublisher {
    ...
    public void publishShipmentCompleted(String shipmentOrderId,
            String invoiceNo, String carrierCd) {
        this.eventPublisher.publishEvent(
            new ShipmentCompletedEvent(shipmentOrderId, invoiceNo, carrierCd,
                DateUtil.now()));
    }
...
public void cancelByShipmentOrder(String shipmentOrderId) {
    ...
    if (packingOrder != null && !"COMPLETED".equals(packingOrder.getStatus())) {
        cancelPackingOrder(packingOrder.getId());
    }

    // 3. 출하 지시 취소
    ShippingOrder shippingOrder = findShippingOrderByShipmentOrder(shipmentOrderId);
    if (shippingOrder != null && !"SHIPPED".equals(shippingOrder.getStatus())) {
        cancelShippingOrder(shippingOrder.getId());
    }

    // 4. 재고 상태 복원 (PICKING → STORED)
...
│                     │                    │                     │
│  피킹 지시 생성 ─────┼── publish ────────→│  RELEASED → PICKING │
│  (PickingStarted)   │                    │                     │
│                     │                    │                     │
...
| 상수명 | 서비스명 | 시점 | 파라미터 |
|--------|---------|------|----------|
| `TRX_FUL_PRE_ASSIGN_PICKING` | `diy-ful-pre-assign-picking-task` | 전 | `{pickingTask, workerCd}` |
| `TRX_FUL_POST_ASSIGN_PICKING` | `diy-ful-post-assign-picking-task` | 후 | `{pickingTask, workerCd, result}` |
...
| 상수명 | 서비스명 | 시점 | 파라미터 |
|--------|---------|------|----------|
| `TRX_FUL_PRE_PRINT_LABEL` | `diy-ful-pre-print-label` | 전 | `{shippingOrder}` |
| `TRX_FUL_POST_PRINT_LABEL` | `diy-ful-post-print-label` | 후 | `{shippingOrder, labelData, result}` |

#### 12.3.10 출하 확정

| 상수명 | 서비스명 | 시점 | 파라미터 |
|--------|---------|------|----------|
| `TRX_FUL_PRE_CONFIRM_SHIPPING` | `diy-ful-pre-confirm-shipping` | 전 | `{shippingOrder}` |
| `TRX_FUL_POST_CONFIRM_SHIPPING` | `diy-ful-post-confirm-shipping` | 후 | `{shippingOrder, result}` |

#### 12.3.11 출하 취소

| 상수명 | 서비스명 | 시점 | 파라미터 |
|--------|---------|------|----------|
| `TRX_FUL_PRE_CANCEL_SHIPPING` | `diy-ful-pre-cancel-shipping` | 전 | `{shippingOrder}` |
| `TRX_FUL_POST_CANCEL_SHIPPING` | `diy-ful-post-cancel-shipping` | 후 | `{shippingOrder, result}` |
...
public class WmsFulfillmentConstants {
    ...
    // === 작업자 배정 ===
    public static final String TRX_FUL_PRE_ASSIGN_PICKING = "diy-ful-pre-assign-picking-task";
    public static final String TRX_FUL_POST_ASSIGN_PICKING = "diy-ful-post-assign-picking-task";
...
public class FulfillmentTransactionController extends AbstractRestService {
     ...
     * 출하 확정 (→ SHIPPED)
     */
    @RequestMapping(value = "shipping_orders/{id}/confirm", method = RequestMethod.POST)
    @ApiDesc(description = "Confirm Shipping")
    public BaseResponse confirmShipping(@PathVariable("id") String id) {
        Long domainId = Domain.currentDomainId();

        // 1. 조회
        ShippingOrder shippingOrder = this.queryManager.select(ShippingOrder.class, id);

        // 2. 커스텀 서비스 - 전 처리
        Map<String, Object> params = ValueUtil.newMap("shippingOrder", shippingOrder);
        this.customSvc.doCustomService(domainId,
            WmsFulfillmentConstants.TRX_FUL_PRE_CONFIRM_SHIPPING, params);

        // 3. 본 로직 실행 (재고 차감 + 이벤트 발행)
        ShippingOrder result = this.fulTrxService.confirmShipping(shippingOrder);
 ...
 */
// params: {shippingOrder: ShippingOrder, result: ShippingOrder}
def shippingOrder = params.get("shippingOrder")

// 특정 화주사는 출하 확정 시 ERP에 즉시 통보
if (shippingOrder.comCd == "COMPANY_A") {
    def erpService = xyz.elidom.sys.util.BeanUtil.get("erpApiService")
    erpService.notifyShipmentConfirmed(
        shippingOrder.shipmentNo, shippingOrder.invoiceNo)
}
```
...
| 출하 지시 | (release_orders에 통합) | `shipping_orders` / `shipping_order_items` ← **신규** |
