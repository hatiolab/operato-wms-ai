...
| `plan_sku` | INTEGER | Y | - | 계획 SKU 종 수 |
| `plan_total` | DOUBLE | Y | - | 계획 총 수량 |
| `result_order` | INTEGER | Y | - | 실적 주문 수 |
| `result_sku` | INTEGER | Y | - | 실적 SKU 종 수 |
...
SELECT
    ...
    pt.worker_id,
    pt.plan_order, pt.plan_sku, pt.plan_total,
...
| POST | `picking_tasks/{id}/start` | 피킹 시작 (CREATED → IN_PROGRESS) |
...
| Method | URL | 설명 |
|--------|-----|------|
| POST | `packing_orders/{id}/print_label` | 운송장 라벨 출력 (COMPLETED → LABEL_PRINTED) |
| POST | `packing_orders/{id}/manifest` | 적하 목록 전송 (LABEL_PRINTED → MANIFESTED) |
| POST | `packing_orders/{id}/confirm_shipping` | 출하 확정 (→ SHIPPED) |
| POST | `packing_orders/confirm_shipping_batch` | 복수 출하 일괄 확정 |
| POST | `packing_boxes/{id}/update_invoice` | 박스별 송장 번호 업데이트 |
...
public void cancelByShipmentOrder(String shipmentOrderId) {
    ...
    if (packingOrder != null && !"SHIPPED".equals(packingOrder.getStatus())) {
        cancelPackingOrder(packingOrder.getId());
    }

    // 3. 재고 상태 복원 (PICKING → STORED)
