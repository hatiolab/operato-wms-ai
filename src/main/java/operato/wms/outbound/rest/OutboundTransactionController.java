...
public class OutboundTransactionController extends AbstractRestService {
     ...
     *  출고 주문 상태    : (REG) 작성 중  ->  (REQ) 요청  -> (WAIT) 출고 접수  ->  (RUN) 작업 중 -> (END) 완료 -> (CANCEL) 취소
     ******************************************************************************************************************/
    ...
    public List<ImportReleaseOrder> importB2CReleaseOrders(@RequestBody List<ImportReleaseOrder> list) {
        ...
        // 3. 커스텀 서비스 호출 - 후 처리
        this.customSvc.doCustomService(domainId, TRX_OUTB_POST_IMPORT_RELEASE, ValueUtil.newMap("releaseOrders", roList));
    ...
    public BaseResponse requestReleaseOrder(@PathVariable("id") String id) {
        ...
        // 3. 커스텀 서비스 호출
        this.customSvc.doCustomService(Domain.currentDomainId(), TRX_OUTB_POST_REQUEST_RELEASE, ValueUtil.newMap("releaseOrder", releaseOrder));
    ...
    public BaseResponse cancelRequestReleaseOrder(@PathVariable("id") String id) {
        ...
        // 3. 커스텀 서비스 호출
        this.customSvc.doCustomService(releaseOrder.getDomainId(), TRX_OUTB_POST_CANCEL_REQUEST_RELEASE, ValueUtil.newMap("releaseOrder", releaseOrder));
    ...
    public BaseResponse acceptSingleReleaseOrder(@PathVariable("id") String id) {
        ...
        // 3. 커스텀 서비스 호출
        this.customSvc.doCustomService(releaseOrder.getDomainId(), TRX_OUTB_POST_ACCEPT_RELEASE, ValueUtil.newMap("releaseOrder", releaseOrder));
    ...
    public BaseResponse cancelAcceptSingleReleaseOrder(@PathVariable("id") String id) {
        ...
        // 3. 커스텀 서비스 호출
        this.customSvc.doCustomService(releaseOrder.getDomainId(), TRX_OUTB_POST_CANCEL_ACCEPT_RELEASE, ValueUtil.newMap("releaseOrder", releaseOrder));
    ...
    public BaseResponse startReleaseOrder(@PathVariable("id") String id) {
        ...
        // 3. 커스텀 서비스 호출
        this.customSvc.doCustomService(releaseOrder.getDomainId(), TRX_OUTB_POST_START_RELEASE, ValueUtil.newMap("releaseOrder", releaseOrder));
    ...
    public BaseResponse cancelStartReleaseOrder(@PathVariable("id") String id) {
        ...
        // 3. 커스텀 서비스 호출
        this.customSvc.doCustomService(releaseOrder.getDomainId(), TRX_OUTB_POST_CANCEL_START_RELEASE, ValueUtil.newMap("releaseOrder", releaseOrder));
    ...
    public BaseResponse startPickingOrder(@PathVariable("id") String id) {
        ...
        // 3. 커스텀 서비스 호출
        this.customSvc.doCustomService(pickingOrder.getDomainId(), TRX_OUTB_POST_START_PICKING, ValueUtil.newMap("pickingOrder", pickingOrder));
    ...
    public BaseResponse pickPickingOrder(@RequestBody Map<String, List<PickingOrderItem>> data) {
        ...
        // 4. 커스텀 서비스 호출
        this.customSvc.doCustomService(pickingOrder.getDomainId(), TRX_OUTB_POST_PICK_PICKING, ValueUtil.newMap("pickingOrder", pickingOrder));
    ...
    public BaseResponse cancelReleaseOrder(@PathVariable("id") String id) {
        ...
        // 3. 커스텀 서비스 호출
        this.customSvc.doCustomService(releaseOrder.getDomainId(), TRX_OUTB_POST_CANCEL_RELEASE, ValueUtil.newMap("releaseOrder", releaseOrder));
    ...
    public BaseResponse cancelReleaseOrderItem(@PathVariable("id") String id, @RequestBody ReleaseOrderItem roItem) {
        ...
        // 3. 커스텀 서비스 호출
        this.customSvc.doCustomService(releaseOrder.getDomainId(), TRX_OUTB_POST_CANCEL_RELEASE_LINE, ValueUtil.newMap("releaseOrder", releaseOrder));
    ...
    public BaseResponse closeReleaseOrder(@PathVariable("id") String id) {
        ...
        // 3. 커스텀 서비스 호출
        this.customSvc.doCustomService(releaseOrder.getDomainId(), TRX_OUTB_POST_CLOSE_RELEASE, ValueUtil.newMap("releaseOrder", releaseOrder));
    ...
    public List<OutboundOrder> searchReleaseOrderItems(
        ...
        // 4. 출고 주문 상세 리스트 조회
        return this.outbTrxService.searchReleaseWorkItems(Domain.currentDomainId(), ValueUtil.toString(filter.getValue()), null, null);
    }
    ...
    public List<OutboundOrder> searchReleaseWorkItems(
        ...
        // 3. 출고 주문 상세 리스트 조회
        List<OutboundOrder> roItems = this.outbTrxService.searchReleaseWorkItems(Domain.currentDomainId(), rlsOrdNo, notCompleted, barcode);
        
        // 4. 출고 작업에 맞게 가공
        for(OutboundOrder item : roItems) {
           String remark = item.getRlsLineNo() + ") [" + item.getLineNo() + "] " + item.getSkuNm() + " / " + item.getOrdQty();
    ...
    public BaseResponse finishReleaseOrderLines(@RequestBody Map<String, List<ReleaseOrderItem>> data) {
        ...
        // 4. 커스텀 서비스 호출
        this.customSvc.doCustomService(releaseOrder.getDomainId(), TRX_OUTB_POST_FINISH_RELEASE_ORDER_LINES, ValueUtil.newMap("releaseOrder,releaseOrderLines", releaseOrder, list));
    ...
    public BaseResponse finishReleaseOrderLine(@PathVariable("release_order_item_id") String releaseOrderItemId, @RequestBody ReleaseOrderItem data) {
        // 1. 출고 주문 상세 조회
        ...
        // 4. 커스텀 서비스 호출
        this.customSvc.doCustomService(ro.getDomainId(), TRX_OUTB_POST_FINISH_RELEASE_ORDER_LINE, ValueUtil.newMap("releaseOrder,releaseOrderLine", ro, roItem));
     ...
     * @return
     */
    @RequestMapping(value = "picking_orders/{id}/download_picking_sheet", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
     ...
     * @return
     */
    @RequestMapping(value = "release_orders/{id}/download_picking_sheet", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiDesc(description = "Download Picking Order Sheet")
    public void downloadForPickingOrderSheefByReleaseOrderId(
        ...
        // 2. 템플릿이 비어 있다면 기본 피킹지시서 템플릿 명 조회
        if(ValueUtil.isEmpty(template)) {
            template = this.outbTrxService.getPickingSheetTemplateName(releaseOrder.getComCd(), releaseOrder.getWhCd(), true);
        }
        
        // 3. 피킹지시서 출력을 위한 PDF 다운로드
        this.printoutCtrl.showPdfByPrintTemplateName(req, res, template, ValueUtil.newMap("releaseOrder", releaseOrder));
    }
     ...
     * @return
     */
    @RequestMapping(value = "release_orders/{id}/download_release_sheet", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    public void downloadForReleaseOrderSheet(
        ...
        // 2. 템플릿이 비어 있다면 기본 피킹지시서 템플릿 명 조회
        if(ValueUtil.isEmpty(template)) {
            template = this.outbTrxService.getTradeStmtSheetTemplateName(releaseOrder.getComCd(), releaseOrder.getWhCd(), true);
        }
