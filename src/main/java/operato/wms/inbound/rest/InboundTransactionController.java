...
public class InboundTransactionController extends AbstractRestService {
     ...
     *  입고 예정 상태    : (INWORK) 작성 중 -> (REQUEST) 요청 -> (READY) 대기  ->  (RUN) 진행 중   -> (END) 완료  
     ********************************************************************************************************/
    ...
    public List<ReceivingItem> finishReceivingOrderLines(
        ...
        String rcvAutoFlag = this.runtimeConfSvc.getRuntimeConfigValue(receiving.getComCd(), receiving.getWhCd(), WmsInboundConfigConstants.RECEIPT_FINISH_AUTO_FLAG);
        if ( ValueUtil.toBoolean(rcvAutoFlag, true) ) {
        	...
        	Map<String, Object> params = ValueUtil.newMap("domainId,receivingId,cancelStatus,endStatus", receiving.getDomainId(), receiving.getId(), WmsInboundConstants.STATUS_CANCEL, WmsInboundConstants.STATUS_END);
    ...
    public ReceivingItem finishReceivingOrderLine(
        ...
        // 5. 후 처리 커스텀 서비스 호출
        this.customSvc.doCustomService(receiving.getDomainId(), TRX_INB_POST_LINE_FINISH_RECEIPT, ValueUtil.newMap("receivingItem", itemToFinish));
    ...
    public List<ReceivingItem> cancelFinishReceivingOrderLines(@RequestBody Map<String, List<ReceivingItem>> data) {
        ...
        List<ReceivingItem> listToFinish = this.queryManager.selectList(ReceivingItem.class, ValueUtil.newMap("domainId,receivingId", receiving.getDomainId(), receiving.getId()));
        
        // 4. 후 처리 커스텀 서비스 호출
        custSvcParams.put("receivingItems", listToFinish);
        this.customSvc.doCustomService(receiving.getDomainId(), TRX_INB_POST_CANCEL_LINES_FINISH_RECEIPT, custSvcParams);
    ...
    public List<ReceivingItem> cancelFinishReceivingOrderLine(@RequestBody ReceivingItem receivingItem) {
        ...
        // 4. 후 처리 커스텀 서비스 호출
        this.customSvc.doCustomService(receiving.getDomainId(), TRX_INB_POST_CANCEL_LINE_FINISH_RECEIPT, ValueUtil.newMap("receivingItems", listToFinish));
     ...
     * @return
     */
    @RequestMapping(value = "receiving_orders/{id}/download_receiving_sheet", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
