 ...
 */
function openMetaNamedSelector(filter, value, confirmCallback) {
  // 그리드 컬럼의 필터 정보를 추출
 ...
 * @param {*} value
 * @param {Object} owner
 * @returns {TemplateResult | string | void}
 */
export const FilterFormMetaNamedSelect: FilterSelectRenderer = (filter, value, owner) => {
  const { options, operator = 'eq' } = filter
  ...
  return operator === 'eq' ? html`
    <input name="${filter.name}" .value=${hiddenText} type="text" hidden />
    <input
      type="text"
      readonly
      ...
      @click=${e => {
        ...
        // 리소스 셀렉터 팝업 실행
        openMetaNamedSelector(filter, value, confirmCallback)
      }}
    />
