...
@customElement('pda-wms-transfer-in-barcode')
export class PdaWmsTransferInBarcode extends connect(store)(PageView) {
  static get styles() {
    return [
      CommonGristStyles,
      CommonHeaderStyles,
  ...
  @query('#oxScanBarcode') private _oxInputBarCode!: OxInputBarcode
  @query('#oxScanLoc') private _oxInputLocation!: OxInputBarcode
  ...
  get context() {
    return {
      title: i18next.t('menu.PdaWmsTransferInBarcode'),
      help: 'operato-wes/pda-wms-transfer-in-barcode',
      actions: [
        {
    ...
    return html`
      ...
      >
        <div class="pda-search-form">
          ...
          <label
            ><span>${TermsUtil.tLabel('loc_cd')}</span>
            <md-icon>edit</md-icon>
          </label>
          <ox-input-barcode
            name="txtLoc"
            id="oxScanLoc"
            placeholder="${TermsUtil.tLabel('scan_location')}"
                        @keydown=${e => {
        this.scanLocCd(e)
      }}
          ></ox-input-barcode>
      ...
      if (ValueUtil.isNotEmpty(tempOrderList) && ValueUtil.isNotEmpty(tempOrderList)) {
        for (let i = 0; i < tempOrderList.length; i++) {
          ...
          let rowDataOrder = {
            ...
            received_qty: tempOrderList[i].received_qty,
            scanned_qty: scanQty,
  ...
  async scanBarCode(e) {
    ...
    //  // 判断单据号是否选择
    //  if (ValueUtil.isEmpty(this._oxInputLocation.value)) {
    
    //   UiUtil.showToast('info', TermsUtil.tText('SHOULD_NOT_BE_EMPTY', { value: TermsUtil.tLabel('loc_cd') }))
    //   return
    ...
    if (ValueUtil.isEmpty(this._orderId)) {
      ...
      UiUtil.showToast('info', TermsUtil.tText('SHOULD_NOT_BE_EMPTY', { value: TermsUtil.tLabel('procurement_order_no') }))
      return
    }
    ...
    let qrCodeData = {
      ...
      scanLocCd: ValueUtil.isNotEmpty(this._oxInputLocation.value)  ?  this._oxInputLocation.value :''
    }

    // 查询对应的拣货数据
    this._pickedInfo = await ServiceUtil.restGet(
      'outbound_work/transfer_orders/receipt/barcode/ready/multi/pda',
      qrCodeData
    )
    ...
    if (ValueUtil.isNotEmpty(this._pickedInfo) && ValueUtil.isNotEmpty(this._pickedInfo.barcode)) {
      ...
      let rowData = {
        ...
        loc_cd: this._pickedInfo.loc_cd,
  ...
  */
  async scanLocCd(e) {
    ...
    // 设置扫描查询数据
    let qrCodeData = {
      whCd:this._orderInfo.to_wh_cd,
      scaLocCd: locCd
    }

    // 查询对应的拣货数据
    let searchLocation = await ServiceUtil.restGet(
      'workhomes/search/peer-to-peer/location/scan',
      qrCodeData
    )
    if (ValueUtil.isNotEmpty(searchLocation)) {
      // LPN输入框
      this.setFocus(true)
    } else {
      // 库位输入框
  ...
  */
  ...
  async save() {
    ...
    let submitModel = { inventorys: this._scanShipmentlList, order_no: this._orderId , receipt_loc_cd: this._oxInputLocation.value}

    await ServiceUtil.restPost(
      'outbound_work/transfer_orders/receipt/barcode/go/multi',
      submitModel,
      'button.save',
      'text.are_you_sure',
      (e: any) => {
  ...
  */
  async updateShipmentStatus(rowReslut: any) {
    if (rowReslut.order_qty == (rowReslut.received_qty + rowReslut.scanned_qty)) {
  ...
  async getScanGristConfig() {
    this.gristConfig = {
      ...
      list: {
        fields: ['barcode', 'product_cd', 'product_nm', 'lot_no', 'picked_qty', 'qty_unit','loc_cd'],
        details: []
      },
      columns: [
    ...
    this.gristConfigOrder = {
      ...
      columns: [
        ...
        },
        {
          ...
          name: 'received_qty',
          label: true,
          header: i18next.t('label.received_qty'),
          hidden: true,
          record: {
            editable: false
          },
  ...
  */
    async resetLocCd(e) {
      // this._oxInputLocation.value = ''
      // this._oxInputLocation.renderRoot.querySelector('input')?.focus()
    }
