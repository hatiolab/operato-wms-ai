...
@customElement('pda-wms-shipment-barcode-manually')
export class PdaWmsShipmentBarcodeManually extends connect(store)(PageView) {
  static get styles() {
    return [
      CommonGristStyles,
      CommonHeaderStyles,
  ...
  @state() private status_picked: string = 'PICKING'
  ...
  // 客户ID
  @state() protected _cust_id: string = ''
  // 客户下拉列表框
  ...
  @property({ type: String }) barcodeReadonly?: boolean = false
  get context() {
    return {
      title: i18next.t('menu.PdaWmsShipmentBarcodeManually'),
      help: 'operato-wes/pda-wms-shipment-barcode-manually',
      actions: [
        {
  ...
  getSearchHtml() {
    return html`
      ...
      <ox-popup-list with-search>
        ...
        <!--客户信息-->
        <label>${TermsUtil.tLabel('cust_cd')}</label>
        <ox-select
          id="oxSelectCustomer"
          data-value
          placeholder=""
          .value=${this._cust_id}
          @change=${(e: Event) => {
        this._cust_id = (e.target as HTMLInputElement).value
      }}
        >
      <ox-popup-list with-search>
            <div option value=''></div> 
            ${this._customerList.map(
        item => html` <div option value=${item.id}>${item.cust_cd} (${item.cust_nm})</div> `
      )}
          </ox-popup-list>
        </ox-select>

        <label>${TermsUtil.tLabel('gi_plan_date')}</label>
        <input
          type="date"
        ...
        />
        <!--出库单号-->
        <label>${TermsUtil.tLabel('gi_order_no')}</label>
        <ox-select
          id="oxSelectOrderNo"
          data-value
          placeholder = '${TermsUtil.tLabel('gi_order_no')}'
          .value=${this._orderId}
          @change=${(e: Event) => {
        let newOrderId = (e.target as HTMLInputElement).value
        if (this._orderId !== newOrderId) {
          this._orderIdChange(newOrderId)
        }
        ...
        item => html` <div option value=${item.gi_order_no}>${item.gi_order_no}</div> `
      )}
          </ox-popup-list>
        </ox-select>
      </div>
  ...
  updated(changes: PropertyValues<this>) {
    ...
    if (changes.has('barcodeReadonly') && !this.barcodeReadonly) {
      // 在禁用状态从true变为false时设置焦点
   ...
   * @param barcode
   */
  async getShipmentList(orderNo: any, rowData: any) {
    ...
    if (ValueUtil.isNotEmpty(this._showOrderList)) {
      ...
      for (let i = 0; i < this._showOrderList.length; i++) {
        if (ValueUtil.isEquals(this._showOrderList[i].status, this.status_all_scaned)) {
          continue
        }

        if (this._showOrderList[i].product_id == rowData.product_id) {
          rowData.line_no = this._showOrderList[i].gi_line_no
      ...
      let orderSeachData = {
        pickingOrderNo: orderNo
      }
      let tempOrderList = await ServiceUtil.restGet(
        'outbound_work/show/order/list/' + orderNo,
        orderSeachData
      )

      console.log(tempOrderList)
      // 判断是否查询到结果
      if (ValueUtil.isNotEmpty(tempOrderList)) {
        for (let i = 0; i < tempOrderList.length; i++) {

          let scanQty = 0;
          if (ValueUtil.isNotEmpty(rowData)) {
            if (tempOrderList[i].gi_line_no == rowData.gi_line_no) {
          ...
          let rowDataOrder = {
            ...
            product_id: tempOrderList[i].product_ref.id,
            product_nm: tempOrderList[i].product_nm,
            product_cd: tempOrderList[i].product_cd,
            package_size_type: tempOrderList[i].package_size_type,
            package_size_type_label: tempOrderList[i].package_size_type,
            ...
            gi_line_no: tempOrderList[i].gi_line_no,
  ...
  async scanBarCode(barcode: any) {
    ...
    try {
      ...
      this._pickedInfo = await ServiceUtil.restGet(
        'outbound_work/picking_orders/release/barcode/ready/multi/pda/manual',
        qrCodeData
      )

      if (ValueUtil.isEmpty(this._pickedInfo.barcode)) {
      ...
      // 检查是否扫码的是同类型产品
      if (!await this.checkAllowManualPicking()) {
        return
      }
      ...
      if (ValueUtil.isNotEmpty(this._pickedInfo) && ValueUtil.isNotEmpty(this._pickedInfo.barcode)) {
        ...
        let rowData = {
          ...
          package_size_type: this._pickedInfo.product_spec4,
...
*/
  async checkScanOrderTotalQty(rowData: any) {
    ...
    if (ValueUtil.isNotEmpty(this._showOrderList) && this._showOrderList.length > 0) {
      ...
      for (let i = 0; i < this._showOrderList.length; i++) {
        ...
        if (!ValueUtil.isEquals(this._showOrderList[i].product_id, rowData.product_id)) {
          continue
        } else {
          if (ValueUtil.isEquals(this._showOrderList[i].status, this.status_all_scaned)) {
            row = i
          ...
          let checkQty = this._showOrderList[i].scanned_qty + this._showOrderList[i].issued_qty + rowData.picked_qty
    ...
    if (!returnValue) {
      await UiUtil.showAlertPopup(
        'title.error',
        TermsUtil.tText('EXCEEDING_THE_DETAILED_ORDER_QTY', { value1: rowData.product_cd + "(" + rowData.product_nm + ")", value2: this._showOrderList[row].line_no, value3: this._showOrderList[row].order_qty }),
        'error',
        'confirm',
        ''
      )
  ...
  /**
   * @description 检查是否是同类产品
   ****************************************
   */
  async checkAllowManualPicking() {
    console.log(this._pickedInfo)
    if (ValueUtil.isNotEmpty(this._showOrderList) && this._pickedInfo) {
      const isProductInList = this._showOrderList.some(
        item => item.product_id === this._pickedInfo.product_id
      );

      if (!isProductInList) {
        await UiUtil.showAlertPopup(
          'title.error',
          TermsUtil.tText('not_same_with_gi_order_items', {}),
          'error',
          'confirm',
          ''
        );
      ...
      const isPackagingInList = this._showOrderList.some(
        item => item.package_size_type === this._pickedInfo.product_spec4
      );

      if (!isPackagingInList) {
        await UiUtil.showAlertPopup(
          'title.error',
          TermsUtil.tText('not_same_with_gi_order_items_pack', {}),
          'error',
          'confirm',
          ''
        );
  ...
  */
  ...
  async save() {
    ...
    await ServiceUtil.restPost(
      'outbound_work/gi_orders/release/manual/barcode/go/multi',
      submitModel,
      'button.save',
      'text.are_you_sure',
      (e: any) => {
  ...
  async delete(rowIndex: any) {
    ...
    if (confirm) {
      ...
      if (ValueUtil.isNotEmpty(this._showOrderList)) {
        ...
        for (let i = 0; i < this._showOrderList.length; i++) {
          if (this._showOrderList[i].scanned_qty <= 0) {
            continue
          }
  ...
  */
  ...
  async getScanGristConfig() {
    this.gristConfig = {
      ...
      list: {
        fields: ['barcode', 'product_cd', 'product_nm', 'lot_no', 'picked_qty', 'qty_unit', 'package_size_type', 'line_no'],
        details: []
      },
      columns: [
        ...
        {
          type: 'select',
          name: 'package_size_type',
          label: true,
          header: i18next.t('label.spec_package_size_type'),
          record: {
            editable: false,
            options: await ServiceUtil.getCodeSelectorData('PACKAGE_SIZE_TYPE')
          },
          width: 80
        },
        {
  ...
  async getPickedOrderList() {
    // 初始化获取拣货单列表
    this._pickedOrderList = await ServiceUtil.restGet('outbound_work/getOrderList/manual/byDate', {
      orderDate: this._releaseOrderDate,
      whCd: this._wcCd,
      custId: this._cust_id,
      giType: 'NORMAL'
    })

    if (ValueUtil.isNotEmpty(this._pickedOrderList)) {
      this._orderInfo = this._pickedOrderList[0]
      this._orderId = this._pickedOrderList[0].picking_order_no
    } else {
      this._orderInfo = {}
      this._orderId = ''
    ...
    this._orderIdChange(this._orderId)
  }
