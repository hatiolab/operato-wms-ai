...
@customElement('pda-wms-shipment-barcode')
export class PdaWmsShipmentBarcode extends connect(store)(PageView) {
  static get styles() {
    return [
      CommonGristStyles,
      CommonHeaderStyles,
  ...
  get context() {
    return {
      title: i18next.t('menu.PdaWmsShipmentBarcode'),
      help: 'operato-wes/pda-wms-shipment-barcode',
      actions: [
        {
  ...
  async getShipmentList(pickOrderNo: any, rowData: any) {
    ...
    if (ValueUtil.isNotEmpty(this._showOrderList)) {
      ...
      let tempOrderList = await ServiceUtil.restGet(
        'outbound_work/picking_orders/release/summary/barcode/show/multi/' + pickOrderNo,
        orderSeachData
      )
  ...
  async scanBarCode(barcode: any) {
    ...
    this._pickedInfo = await ServiceUtil.restGet(
      'outbound_work/picking_orders/release/barcode/ready/multi/pda',
      qrCodeData
    )
  ...
  */
  ...
  async save() {
    ...
    await ServiceUtil.restPost(
      'outbound_work/picking_orders/release/barcode/go/multi',
