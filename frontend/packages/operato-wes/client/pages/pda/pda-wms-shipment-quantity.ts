...
@customElement('pda-wms-shipment-quantity')
export class PdaWmsShipmentQuantity extends connect(store)(PageView) {
  static get styles() {
    return [
      CommonGristStyles,
      CommonHeaderStyles,
  ...
  get context() {
    return {
      title: i18next.t('menu.PdaWmsShipmentQuantity'),
      help: 'operato-wes/pda-wms-shipment-quantity',
      actions: [
        {
  ...
  getSearchHtml() {
    return html`
      ...
      <ox-popup-list with-search>
        ...
        <label>${TermsUtil.tLabel('order_date')}</label>
        <input
          type="date"
        ...
        />
        <!--拣货单号-->
        <label>${TermsUtil.tLabel('picking_order_no')}</label>
        <ox-select
          id="oxSelectOrderNo"
          data-value
          placeholder = '${TermsUtil.tLabel('picking_order_no')}'
          .value=${this._orderId}
          @change=${(e: Event) => {
        ...
        >
          <ox-popup-list with-search>
            ...
            ${this._pickedOrderList.map(
              item => html` <div option value=${item.picking_order_no}>${item.picking_order_no}</div> `
            )}
          </ox-popup-list>
        </ox-select>
      </div>
    ...
    return html`
      ...
      >
        <div class="pda-search-form">
          <label
            ><span>*${TermsUtil.tLabel('product_id')}</span>
            <md-icon>edit</md-icon>
          </label>
          <ox-input-barcode
            name="txtQrCode"
            id="oxScanBarcode"
            placeholder="${TermsUtil.tLabel('scan_product')}"
            @change=${e => {
  ...
  async getShipmentList(pickOrderNo: any, rowData: any) {
    ...
    if (ValueUtil.isNotEmpty(this._showOrderList)) {
      ...
      for (let i = 0; i < this._showOrderList.length; i++) {
        if (this._showOrderList[i].gi_line_no == rowData.gi_line_no) {
      ...
      let tempOrderList = await ServiceUtil.restGet(
        'outbound_work/picking_orders/release/summary/product/show/multi/' + pickOrderNo,
        orderSeachData
      )
      ...
      if (ValueUtil.isNotEmpty(tempOrderList) && ValueUtil.isNotEmpty(tempOrderList)) {
        for (let i = 0; i < tempOrderList.length; i++) {
          ...
          let rowDataOrder = {
            ...
            picked_qty: tempOrderList[i].picked_qty,
  ...
  async scanBarCode(barcode: any) {
    ...
    this._pickedInfo = await ServiceUtil.restGet(
      'outbound_work/picking_orders/release/qty/ready/multi/pda',
      qrCodeData
    )
    ...
    // 判断是否查询到结果
    if (ValueUtil.isNotEmpty(this._pickedInfo) && ValueUtil.isNotEmpty(this._pickedInfo.gi_order_no)) {
      let rowData = {
        id: this._pickedInfo.id,
        product_cd: this._pickedInfo.product_cd,
        product_nm: this._pickedInfo.product_nm,
        barcode: this._pickedInfo.barcode,
        lot_no: this._pickedInfo.lot_no,
        gi_line_no: this._pickedInfo.gi_line_no,
        reserved_qty: this._pickedInfo.reserved_qty,
        picked_qty: this._pickedInfo.picked_qty,
        picking_order_no: this._pickedInfo.picking_order_no,
  ...
  */
  ...
  async save() {
    ...
    await ServiceUtil.restPost(
      'outbound_work/picking_orders/release/qty/go/multi',
  ...
  async delete(rowIndex: any) {
    ...
    if (confirm) {
      ...
      if (ValueUtil.isNotEmpty(this._showOrderList)) {
        ...
        for (let i = 0; i < this._showOrderList.length; i++) {
          if (this._showOrderList[i].gi_line_no == rowResult.gi_line_no) {
  ...
  */
  ...
  async getScanGristConfig() {
    this.gristConfig = {
      ...
      list: {
        fields: ['product_cd', 'product_nm', 'lot_no', 'picked_qty', 'qty_unit'],
        details: []
      },
      columns: [
   ...
   */
   checkDataDuplicated(records, item) {
    if (records.length == 0) {
      ...
      if (duplicatedItem) {
        UiUtil.showToast('info', 'This ProdhctCd [' + item.barcode + '] is already scanned!')
        return null
      } else {
        return item
      }
  ...
  async getPickedOrderList() {
    // 初始化获取拣货单列表
    this._pickedOrderList = await ServiceUtil.restGet('picking_orders/getPickedOrderList/byDate', {
