 ...
 * PDA B2C 전용 검수/포장 화면
 *
 * pda-fulfillment-packing.js 기반 B2C 전용 버전:
 * - biz_type = B2C_OUT 고정
 * - 웨이브 일자 + 차수로 포장 주문 조회 (초기 자동 조회 없음)
 * - 운송장번호 항상 필수 (B2B 분기 없음)
 * - 거래명세서 출력 없음, 배송 라벨 출력 제공
 */
@customElement('pda-fulfillment-b2c-packing')
export class PdaFulfillmentB2cPacking extends connect(store)(PageView) {
  /** 화면 모드: list / inspection / packing / complete */
  @state() mode = 'list'

  /** 웨이브 일자 */
  @state() waveDate = ValueUtil.todayFormatted()
  /** 웨이브 차수 */
  @state() waveSeq = ''
  ...
  static get styles() {
    return [
      ...
      css`
        ...
        .wave-filter input:focus {
          outline: none;
          border-color: var(--md-sys-color-primary, #1976D2);
        }

        .wave-filter .btn-search {
          flex-shrink: 0;
          height: 36px;
          padding: 0 12px;
          border: none;
        ...
        .wave-filter .btn-search:active {
          opacity: 0.85;
        }

        /* 검색 조건 하단 주문 진행 프로그레스 바 */
  ...
  get context() {
    return {
      title: TermsUtil.tMenu('FulfillmentB2CPackingWork')
    }
  }
  ...
  _renderHeader() {
    const invoiceNo = this.selectedOrder?.invoice_no || ''
  ...
  /** list 모드 렌더링 — 웨이브 필터, 현황 요약, 포장 지시 목록, 포장번호 스캔 */
  _renderListMode() {
    ...
    return html`
      <div class="wave-filter">
        <input
          ...
          title="웨이브 일자"
          .value="${this.waveDate}"
          @change=${e => { this.waveDate = e.target.value }}
        />
        <input
          type="number"
          min="1"
          ...
          title="웨이브 차수"
          .value="${this.waveSeq}"
          @input=${e => { this.waveSeq = e.target.value }}
          @keydown=${e => { if (e.key === 'Enter') this._loadPackingOrders() }}
        />
      ...
      ${this.loading
        ...
        : html`
          <div class="order-list">
            ...
            ? html`<div class="empty-message">${this.totalOrderCount === 0 ? '웨이브 일자와 차수를 입력하고 검색하세요' : '포장 작업이 없습니다'}</div>`
    ...
    return html`
      <div class="order-card" @click=${() => this._selectOrder(order)}>
        <div class="card-header">
          <span class="pack-no">송장번호: ${order.invoice_no}</span>
        ...
        <div class="sub-info">
          ${order.shipment_no} · ${order.cust_nm || ''} · ${totalItems}종 ${order.total_qty || 0}EA
        </div>
        ${isInProgress ? html`
  ...
  /** packing 모드 렌더링 — 포장 정보 입력 + 운송장 (B2C 항상 필수) + 출고 확정 */
  _renderPackingMode() {
    return html`
      ...
      <button class="btn-confirm"
        ?disabled=${this.processing || !this.trackingNo.trim()}
  ...
  /** complete 모드 렌더링 — 완료 통계 + 라벨 출력 + 다음작업/목록 버튼 */
  _renderCompleteMode() {
    ...
    return html`
      <div class="complete-section">
        ...
        <div class="result-card">
          ...
          <div><span class="label">${TermsUtil.tLabel('wave_no') || '웨이브 번호'}:</span> ${this.selectedOrder?.wave_no}</div>
          ...
          <div><span class="label">${TermsUtil.tLabel('cust_nm') || '수취인'}:</span> ${this.selectedOrder?.cust_nm + ' (' + this.selectedOrder?.tel1 + ')'}</div>
        ...
        <div class="btn-group">
          <button class="btn-label" @click=${this._printLabel}>
            ${TermsUtil.tButton('export-label') || '배송 라벨 출력'}
          </button>
          ${!isViewMode ? html`
  ...
  /** 페이지 초기화 — 자동 조회 없음 (사용자가 웨이브 조건 입력 후 검색) */
  pageInitialized() { }

  /* ==================== Data Loading ==================== */

  /** 포장 지시 목록 조회 (B2C_OUT, 웨이브 일자/차수 필수) */
  async _loadPackingOrders(page = 1) {
    if (!page || typeof page !== 'number') page = 1

    if (!this.waveDate || !this.waveSeq) {
      document.dispatchEvent(new CustomEvent('notify', {
        detail: { level: 'warn', message: '웨이브 일자와 차수를 입력해주세요' }
      }))
      return
    }
    this.loading = true
  ...
  async _onScanPackingOrder(barcode) {
    ...
    if (!this.waveDate || !this.waveSeq) {
      document.dispatchEvent(new CustomEvent('notify', {
        detail: { level: 'warn', message: '웨이브 일자와 차수를 먼저 입력해주세요' }
      }))
      return
    }
