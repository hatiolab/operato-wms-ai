 ...
 * @description [Goods Issue Picking] Product Based Picking Manual Page
 */
export class GiProductManualPick extends GristRenderMixin(p13n(localize(i18next)(PageView))) {
  /**
  ...
  async reset_search_form() {
    this.findSearchFormInput('product_cd').value = ''
  ...
  async fetchHandler(fetchParam) {
    ...
    let validDataList = this.checkDataDuplicated(records, results.records)
    // 4. reset barcode & focus on
    this.resetBarcodeAndFocus()
    // 5. scanned data is valid then
    if (validDataList && validDataList.length > 0) {
      validDataList.forEach(validData => records.push(validData))
    }
    // 6. return results
  ...
  resetBarcodeAndFocus() {
    let productElement = this.findSearchFormInput('product_cd')
...
customElements.define('gi-product-manual-pick', GiProductManualPick)
