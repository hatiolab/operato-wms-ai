 ...
 * @description [Goods Issue Picking] Barcode Based Picking Manual Picking
 */
export class GiBarcodeManualPick extends GristRenderMixin(p13n(localize(i18next)(PageView))) {
  /**
  ...
  async firstUpdated() {
    ...
    let pickOrderEle = this.findSearchFormInput('picking_order_no')
    let pickOrderNo = pickOrderEle.value
    if (!pickOrderNo || pickOrderNo == '') {
      pickOrderEle.focus()
    }
  }
  ...
  async fetchHandler(fetchParam) {
    // 1. scanned barcode data
    let records = await super.fetchHandler(fetchParam)

    // 2. form value check
    let giOrderNo = this.findSearchFormInput('picking_order_no').value
    let barcode = this.findSearchFormInput('barcode').value

    // 3. if giOrderNo does not exist -> error
    if (!giOrderNo) {
      return { records: [], total: 0 }
    }

    // 4. if barcode is empty then gi order item summary data
    if (!barcode) {
      return records

      // 5. if barcode is not empty then show barcode information
    } else {
      // 5.1 reset barcode & focus on
...
customElements.define('gi-barcode-manual-pick', GiBarcodeManualPick)
