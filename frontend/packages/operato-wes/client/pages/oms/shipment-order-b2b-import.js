...
/**
 * B2B 출고 주문 임포트 - 3단계 마법사
 *
 * Step 1: 파일 업로드
 * Step 2: 검증/미리보기 (거래처·비고 컬럼, 배송지 정보 없음)
 * Step 3: 결과 확인
 */
class ShipmentOrderB2BImport extends localize(i18next)(PageView) {
  /** 컴포넌트 스타일 정의 */
  static get styles() {
    return [
      css`
  ...
  /** 페이지 컨텍스트 반환 */
  get context() {
    return { title: TermsUtil.tMenu('ImportShipmentOrderB2B') || 'B2B 출고 주문 임포트' }
  }

  /** 화면 렌더링 */
  render() {
    return html`
      <h2>${TermsUtil.tMenu('ImportShipmentOrderB2B') || 'B2B 출고 주문 임포트'}</h2>
  ...
  _renderStep1() {
    return html`
      <h3 style="margin-top:0; margin-bottom:20px; font-size:16px; color:#424242;">Step 1: 파일 업로드 (B2B)</h3>
      ...
      <div class="template-buttons">
        <button class="template-btn" @click="${() => this._downloadTemplate()}">📥 B2B 템플릿 다운로드</button>
      </div>
  ...
  /** Step 2: B2B 검증/미리보기 — 거래처·비고 컬럼, 배송지 정보 없음 */
  _renderStep2() {
    ...
    return html`
      <h3 style="margin-top:0; margin-bottom:20px; font-size:16px; color:#424242;">Step 2: 검증/미리보기 (B2B)</h3>
  ...
  /** Step 1 → Step 2: B2B 서버 검증 요청 */
  async _goToStep2() {
    if (!this.parsedData || this.parsedData.length === 0) return

    try {
      this.processing = true
      const result = await ServiceUtil.restPost('oms_trx/shipment_orders/import/excel/b2b', this.parsedData)
      if (result) {
  ...
  async _downloadTemplate() {
    const settingName = 'template.outbound.b2b.order'
    try {
...
window.customElements.define('shipment-order-b2b-import', ShipmentOrderB2BImport)
