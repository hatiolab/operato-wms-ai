...
/**
 * B2C 출고 주문 임포트 - 3단계 마법사
 *
 * Step 1: 파일 업로드
 * Step 2: 검증/미리보기 (판매채널·배송지·수신자 컬럼, 거래처 정보 없음)
 * Step 3: 결과 확인
 */
class ShipmentOrderB2CImport extends localize(i18next)(PageView) {
  /** 컴포넌트 스타일 정의 */
  static get styles() {
    return [
      css`
  ...
  /** 페이지 컨텍스트 반환 */
  get context() {
    return { title: TermsUtil.tMenu('ImportShipmentOrderB2C') || 'B2C 출고 주문 임포트' }
  }

  /** 화면 렌더링 */
  render() {
    return html`
      <h2>${TermsUtil.tMenu('ImportShipmentOrderB2C') || 'B2C 출고 주문 임포트'}</h2>
  ...
  _renderStep1() {
    return html`
      <h3 style="margin-top:0; margin-bottom:20px; font-size:16px; color:#424242;">Step 1: 파일 업로드 (B2C)</h3>
      ...
      <div class="template-buttons">
        <button class="template-btn" @click="${() => this._downloadTemplate()}">📥 B2C 템플릿 다운로드</button>
      </div>
  ...
  /** Step 2: B2C 검증/미리보기 — 판매채널·배송지·수신자 컬럼 포함 */
  _renderStep2() {
    ...
    return html`
      <h3 style="margin-top:0; margin-bottom:20px; font-size:16px; color:#424242;">Step 2: 검증/미리보기 (B2C)</h3>
  ...
  /** Step 1 → Step 2: B2C 서버 검증 요청 */
  async _goToStep2() {
    if (!this.parsedData || this.parsedData.length === 0) return

    try {
      this.processing = true
      const result = await ServiceUtil.restPost('oms_trx/shipment_orders/import/excel/b2c', this.parsedData)
      if (result) {
  ...
  async _downloadTemplate() {
    const settingName = 'template.outbound.b2c.order'
    try {
...
window.customElements.define('shipment-order-b2c-import', ShipmentOrderB2CImport)
