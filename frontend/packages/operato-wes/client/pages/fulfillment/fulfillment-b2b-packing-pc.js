...
/**
 * 풀필먼트 B2B 전용 검수/포장 PC 화면
 *
 * fulfillment-packing-pc.js 기반의 B2B 전용 버전:
 * - biz_type = B2B_OUT 고정 (B2C 주문 제외)
 * - 운송장번호 입력 없음 (B2B는 거래명세서로 대체)
 * - 출고 확정 + 거래명세서 출력 버튼 항상 표시
 *
 * 작업 흐름:
 ...
 */
class FulfillmentB2bPackingPc extends localize(i18next)(PageView) {
  /** 컴포넌트 스타일 정의 */
  static get styles() {
    return [
      css`
  ...
  constructor() {
    ...
    this.orderDate = ValueUtil.todayFormatted()
  ...
  get context() {
    return {
      title: TermsUtil.tMenu('FulfillmentB2BPackingPc')
    }
  }
  ...
  /** 좌측 패널 렌더링 - 검색, 필터, 진행 프로그레스, 포장 주문 카드 목록 (서버 페이지네이션) */
  _renderLeftPanel() {
      ...
      <div class="left-panel">
        ...
        <div class="search-filters">
          <input
            ...
            @change="${e => { this.orderDate = e.target.value; this.listPage = 1 }}"
          />
  ...
  /** 포장 주문 카드 렌더링 - 포장번호, 상태 배지, 출고번호/고객명/수량/작업장, 진행률 표시 */
  _renderOrderCard(order) {
    const isSelected = this.selectedOrder && this.selectedOrder.id === order.id
    const isCompleted = !['CREATED', 'IN_PROGRESS'].includes(order.status)
    const isInProgress = order.status === 'IN_PROGRESS'
    const itemCount = order.total_items || 0
    const totalQty = order.total_qty || 0
    const progressPct = itemCount > 0 ? Math.round(((order.packed_qty || 0) / itemCount) * 100) : 0

    const statusClass = (order.status || '').toLowerCase()
    const statusLabel = order.status === 'CREATED'
    ...
    return html`
      ...
      >
        <div class="card-header">
          <span class="pack-no">포장번호: ${order.pack_order_no || '-'}</span>
    ...
    return html`
      <div class="right-panel">
        <div class="empty-panel">
          ...
          <span class="sub">또는 포장번호 / 출고번호를 스캔하세요</span>
          <input
            type="text"
            placeholder="포장번호 / 출고번호 스캔"
  ...
  /* ===== 포장 화면 (B2B: 운송장 없음, 거래명세서 출력) ===== */

  /** 포장 패널 렌더링 - 포장 정보 입력, 출고 확정 + 거래명세서 출력 버튼 */
  _renderPackingPanel() {
    const order = this.selectedOrder
    if (!order) return this._renderEmptyPanel()
      ...
      <div class="right-panel">
        ...
        <div class="right-panel-content">
          ...
          <div class="packing-section">
            ...
            <div class="info-card">
              <h4>주문 정보</h4>
              <div class="info-row"><span class="label">거래처</span><span class="value">${order.cust_nm || '-'}</span></div>
              <div class="info-row"><span class="label">주문번호</span><span class="value">${order.shipment_no || '-'}</span></div>
  ...
  /** 완료 패널 렌더링 - 출고 완료 통계, 거래명세서 출력, 다음 검수 시작 */
        ...
        <div class="right-panel-content">
          <div class="complete-panel">
            ...
            <div class="complete-stats">
              <div class="stat-row"><span class="label">주문 번호</span><span class="value">${order.shipment_no}</span></div>
            ...
            <div class="complete-actions">
              ...
              <button class="btn-action" @click="${this._printDeliveryStatement}">거래명세서 출력</button>
  ...
  async _refresh(page = 1) {
    ...
    try {
      ...
      const result = await ServiceUtil.restGet(`ful_trx/packing_orders/list?${params}`).catch(() => ({}))
  ...
  async _onScanPackingOrder(barcode) {
    ...
    try {
      ...
      if (order) {
        this._onSelectOrder(order)
      } else {
        this._showFeedback('error', `포장번호를 찾을 수 없습니다: ${barcode}`)
      }
    } catch (error) {
  ...
  /** 검수 수량 직접 입력 처리 - 수량 충족 시 자동 검수 확인 */
  ...
  _moveToNextItem() {
    const nextIdx = this.packingItems.findIndex(
      (item, idx) => idx > this.currentItemIndex && item.status !== 'COMPLETED' && item.status !== 'SHORT'
    )

    if (nextIdx >= 0) {
      this.currentItemIndex = nextIdx
    } else {
      const firstIdx = this.packingItems.findIndex(item => item.status !== 'COMPLETED' && item.status !== 'SHORT')
      this.currentItemIndex = firstIdx >= 0 ? firstIdx : -1
    }
  }

  /** 검수 완료 처리 - 포장 정보 입력 화면으로 전환, 출고 확정 버튼에 포커스 */
  _onInspectionComplete() {
    ...
    setTimeout(async () => {
      await this.updateComplete
      const btn = this.shadowRoot?.getElementById('releaseBtn')
      if (btn) btn.focus()
    }, 100)
  }
  ...
  /** 출고 확정 - B2B는 운송장 불필요, 포장 정보만으로 완료 처리 */
  ...
  /** 다음 검수 시작 - 서버 재조회 후 대기 중인 다음 포장 주문 자동 선택 */
...
window.customElements.define('fulfillment-b2b-packing-pc', FulfillmentB2bPackingPc)
